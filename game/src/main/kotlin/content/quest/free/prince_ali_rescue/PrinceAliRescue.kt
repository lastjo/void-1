...
class PrinceAliRescue(private val fontDefinitions: FontDefinitions) {
    ...
    @Option("Open", "draynor_prison_door_closed")
    suspend fun operate(player: Player, target: GameObject) {
        if (player.inventory.contains("bronze_key_prince_ali_rescue") || player.quest("prince_ali_rescue") == "prince_ali_disguise") {
            when (player.quest("prince_ali_rescue")) {
                ...
                "joe_beers" -> player.dialogue { statement("You'll need to deal with Lady Keli before freeing the Prince.") }
                else -> player.dialogue { statement("You'll need to deal with Lady Keli and the guard before freeing the Prince.") }
            }
        } else {
            player.sound("locked")
    ...
    @Subscribe("quest_journal", "prince_ali_rescue")

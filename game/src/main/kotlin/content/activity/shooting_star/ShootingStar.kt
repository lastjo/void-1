...
class ShootingStar(
    ...
    @Command("star [minutes]", description = "start a new shooting star event in [minutes]")
    fun startStar(player: Player, content: String) {
        cleanseEvent(true)
    ...
    @Spawn
    fun restartBonus(player: Player) {
    ...
    fun starBonus(player: Player): Int = TimeUnit.SECONDS.toTicks(1)

    @TimerTick("shooting_star_bonus_ore_timer")
    fun bonusDeplete(player: Player): Int {
    ...
    fun stopBonus(player: Player) {
        player.message("<dark_red>The ability to mine an extra ore has worn off.")
    }

    @Despawn("shooting_star_tier_1")
    fun despawn(obj: GameObject) {
        areaSound("star_meteor_despawn", obj.tile, radius = 15)
        cleanseEvent(false)
    }

    @TimerStart("mining")
    fun startMining(player: Player): Int {
        val target = (player.mode as? Interact)?.target as? GameObject ?: return -1
    ...
    @Option("Prospect", "crash_star_tier_*")
    ...
    @Option("Talk-to", "star_sprite")
    suspend fun sprite(player: Player, sprite: NPC) = player.talkWith(sprite) {

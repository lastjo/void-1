...
class MakeAmount {
    ...
    @Interface("1", "create1", "skill_creation_amount")
    fun one(player: Player) {
        player["skill_creation_amount", false] = 1
    }

    @Interface("5", "create5", "skill_creation_amount")
    fun five(player: Player) {
        player["skill_creation_amount", false] = 5
    }

    @Interface("10", "create10", "skill_creation_amount")
    fun ten(player: Player) {
        player["skill_creation_amount", false] = 10
    }

    @Interface(component = "all", id = "skill_creation_amount")
    fun all(player: Player) {
        val max: Int = player["skill_creation_maximum", 1]
        player["skill_creation_amount", false] = max
    }

    @Interface("+1", "increment", "skill_creation_amount")
    fun increment(player: Player) {
    ...
    @Interface("-1", "decrement", "skill_creation_amount")
    fun decrement(player: Player) {
        var current: Int = player["skill_creation_amount", 1]
        current--
        if (current < 0) {
            current = 0

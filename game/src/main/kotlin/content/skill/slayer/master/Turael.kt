...
class Turael(private val slayerDefinitions: SlayerTaskDefinitions) {
    ...
    @Option("Talk-to", "turael")
    suspend fun takl(player: Player, target: NPC) = player.talkWith(target) {
    ...
    @Option("Get-task", "turael")
    suspend fun task(player: Player, target: NPC) = player.dialogue {
        assignTask()
    }
    ...
    @Option("Rewards", "turael")
    fun rewards(player: Player, target: NPC) {
        player.open("slayer_rewards_learn")
    }

...
class MatureAle {
    ...
    fun asgarnian(player: Player, item: Item): Boolean {
    ...
    fun axemans(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
    ...
    fun chefs(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
    ...
    @Consume("*cider")
    fun cider(player: Player, item: Item): Boolean {
    ...
    fun bitter(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
        player.levels.boost(Skill.Strength, if (mature) 3 else 2)
        player.levels.drain(Skill.Attack, if (mature) 6 else 4)
        return false
    ...
    fun stout(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
    ...
    fun ale(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
    ...
    fun respite(player: Player, item: Item): Boolean {
        val mature = item.id.endsWith("_m")
    ...
    fun mindBomb(player: Player, item: Item): Boolean {
        val mature = item.id.startsWith("mature_")

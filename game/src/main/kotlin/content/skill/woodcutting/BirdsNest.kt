...
class BirdsNest(
    private val drops: DropTables,
    private val itemDefinitions: ItemDefinitions,
) {

    @Inventory("Search")
    fun search(player: Player, item: Item, itemSlot: Int) {
        ...
        val table = drops.get(tableId) ?: return
        val items = mutableListOf<ItemDrop>()
        table.role(list = items)

        val drop = items.firstOrNull() ?: return
        val itemId = drop.id
        ...
        if (player.inventory.replace(itemSlot, item.id, "birds_nest_empty")) {

...
class GnomeAdvanced(private val npcs: NPCs) {

    @Option("Climb-up", "gnome_tree_branch_advanced")
    suspend fun climbBranch(player: Player, target: GameObject) {
        if (!player.has(Skill.Agility, 85, message = true)) {
            player.dialogue { npc<Happy>("gnome_trainer", "Sorry mate, you're not experienced enough to try that route. I suggest you carry on over the balancing rope instead.") }
            return
        }
    ...
    @Option("Run-across", "gnome_sign_post_advanced", approach = true)
    suspend fun runAcrossSign(player: Player, target: GameObject) {
    ...
    @Option("Swing-to", "gnome_pole_advanced", approach = true)
    suspend fun swingToPole(player: Player, target: GameObject) {
    ...
    @Option("Jump-over", "gnome_barrier_advanced")
    suspend fun jumpBarrier(player: Player, target: GameObject) {

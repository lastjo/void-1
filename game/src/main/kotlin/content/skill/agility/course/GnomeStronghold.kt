...
class GnomeStronghold(private val npcs: NPCs) {

    @Option("Walk-across", "gnome_log_balance")
    suspend fun crossLog(player: Player, target: GameObject) {
    ...
    suspend fun climbNet(player: Player, target: GameObject) {
        player.arriveDelay()
        player.agilityCourse("gnome")
    ...
    @Option("Walk-on", "gnome_balancing_rope")
    suspend fun walkRope(player: Player, target: GameObject) {
    ...
    @Option("Walk-on", "gnome_balancing_rope_end")
    suspend fun walkRopeBack(player: Player, target: GameObject) {
        player.renderEmote("rope_balance")
    ...
    @Option("Climb-down", "gnome_tree_branch_down")
    suspend fun climbBranchDown(player: Player, target: GameObject) {
        player.message("You climb the tree...", ChatType.Filter)
    ...
    @Option("Climb-over", "gnome_obstacle_net_free_standing")
    suspend fun climbOverNet(player: Player, target: GameObject) {
        player.agilityCourse("gnome")
    ...
    @Option("Squeeze-through", "gnome_obstacle_pipe_*")
    suspend fun squeezeThroughPipe(player: Player, target: GameObject): Unit = player.run {
        agilityCourse("gnome")
        delay()

...
class OuraniaAltar(private val drops: DropTables) {

    val logger = InlineLogger()

    @Option("Craft-rune", "ourania_altar")
    fun operate(player: Player, target: GameObject) {
        val level = player.levels.get(Skill.Runecrafting)
        val table = drops.get("ourania_rune_table_level_${if (level >= 99) 10 else level / 10}") ?: return
        var experience = 0.0

...
class Eniola {
    ...
    @Option("Bank", "eniola")
    suspend fun bank(player: Player, npc: NPC) = player.talkWith(npc) {
        openBank()
    }

    @Option("Collect", "eniola")
    suspend fun collect(player: Player, npc: NPC) = player.talkWith(npc) {
        openCollection()
    }
    ...
    fun open(player: Player, id: String) {
        ...
        player.interfaces.sendText(id, "text", "Choose a highlighted rune to make your payment.")
    }

    @Continue("ourania_bank_charge", "*_rune")
    fun resume(player: Player, component: String) {
        (player.dialogueSuspension as? StringSuspension)?.resume(component)
    }

    @Interface(component = "*_rune", id = "ourania_bank_charge")
    fun select(player: Player, id: String) {
        if (player.inventory.remove(id, 20)) {
            player.open(player["ourania_interface", "bank"])
        } else {

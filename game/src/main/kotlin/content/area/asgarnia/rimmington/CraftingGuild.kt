...
class CraftingGuild {
    ...
    @Option("Open", "guild_door_2_closed")
    suspend fun enter(player: Player, target: GameObject) = player.dialogue {
    ...
    @Option("Talk-to", "master_crafter_2")
    suspend fun welcome(player: Player, npc: NPC) = player.talkWith(npc) {
        npc<Happy>("Hello, and welcome to the Crafting Guild. Accomplished crafters from all over the land come here to use our top notch workshops.")
    }

    @Option("Talk-to", "master_crafter_3")
    suspend fun grumpy(player: Player, npc: NPC) = player.talkWith(npc) {
        npc<Uncertain>("Yeah?")
        player<Happy>("Hello.")

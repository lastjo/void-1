...
interface Character :
     ...
     * Note: only applies to @Option and @UseOn subscriptions
     */
    ...
    private suspend fun delayTarget(target: Tile) {
        ...
        if (tile.distanceTo(target) >= 50) {
            logger.warn { "Skipping target delay distance too great char=$this, target=$target, context=${this@Character}" }
            return
        }
        ...
        if (count >= 50) {
            logger.warn { "Failed delaying target char=$this, target=$target, context=${this@Character}" }
        }
    }
